...
static bool is_pushing_box = false;
static double initial_box_distance = 0.0;
...
static void run_braitenberg() {
  ...
  //empurra a caixa com distancia maior que zero
  for(i=0; i < DISTANCE_SENSORS_NUMBER; i++){
    if(distance_sensors_values[i]>0.1){
      is_pushing_box = true;
      initial_box_distance = distance_sensors_values[i];
      break;
    }
  }
...
// Se a caixa se moveu uma certa distância (por exemplo, a distância diminuiu em mais de 0.1),
// o robô para e acende os leds
for (int i = 0; i < DISTANCE_SENSORS_NUMBER; i++) {
if (distance_sensors_values[i] > 0.9 && distance_sensors_values[i] < initial_box_distance - 0.1) {
